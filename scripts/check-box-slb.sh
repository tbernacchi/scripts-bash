...
fn_send_zabbix_slb_tt_reqs_bytes()
	{
        $ZBXSND -z $ZBXPRX -s $vip_name -k SLB_TT_REQS_BYTES -o $SLB_TT_REQS_BYTES
	}
...
fn_check_status_system_by_vip()
	...
	for vip_name in `echo $VIP_NAMES`
		do
			...
			SLB_TT_REQS_BYTES=`cat $TEMP_FILE | grep $vip_name -A17 | tail -n1 | awk '{ print $2 }' | sed -e 's/"//g' -e 's/,//g'`
			fn_send_zabbix_slb_tt_reqs_bytes
