...
SLACK_NOC_API="https://hooks.slack.com/services/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"
...
fn_check_close_im()
	...
	#STATUS=`curl -D- -u svc_zabbix_monit@tabajara.com.br:xEQD5hHyy0hEUBrqJvXN2616 -i -X GET -H 'Content-Type: application/json' $JIRA_API/$ISSUE/status | grep size | jq ".values" | grep -w status | awk '{ print $2 }' | cut -f1 -d"," | sed 's/"//g'`
	STATUS=`curl -D- -u svc_zabbix_monit@tabajara.com.br:xEQD5hHyy0hEUBrqJvXN2616 -i -X GET -H 'Content-Type: application/json' $JIRA_API/$ISSUE/status |  grep size | jq .values | grep -w status | awk '{ print $2 }' | sed 's/"//g' | cut -f1 -d,`
	sleep .5
...
END
	...
	curl -v -D- -u svc_zabbix_monit@tabajara.com.br:xEQD5hHyy0hEUBrqJvXN2616 -i -X POST -H 'Content-Type: application/json' -d@$JSON_DONE https://sd-tabajara.atlassian.net/rest/api/2/issue/$ISSUE/transitions
	sleep .5
	...
	curl -v -D- -u svc_zabbix_monit@tabajara.com.br:xEQD5hHyy0hEUBrqJvXN2616 -i -X POST -H 'Content-Type: application/json' -d@$JSON_CLOSE https://sd-tabajara.atlassian.net/rest/api/2/issue/$ISSUE/transitions
	sleep .5
...
echo $PRIO_SD
	if [ $COD_PRIO = 5 ]
		then
			...
			curl -v -D- -u svc_zabbix_monit@tabajara.com.br:xEQD5hHyy0hEUBrqJvXN2616 -i -X POST -H 'Content-Type: application/json' -d@$JSON_SD $JIRA_API >> $WORKDIR/$FILE_DESCRIPTION.description
			echo "Se chegou era novo $trigger"
      
      # mando para o slack
      grep -w "hostname" $JSON_SLACK > /dev/null
      RESULT=`echo $?`
