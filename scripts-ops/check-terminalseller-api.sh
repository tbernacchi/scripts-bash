...
AGR="termialseller-api"
KEY="PROBE"
FILE="/tmp/falha-probe-tseller"
...
for SRV in `echo $SERVERS`
	do
		RESULT=`curl -s --connect-timeout 5 --header 'Host: credenciamento.tabajara.com.br' http://$SRV/terminalseller-api/health | grep -w  "UP"`

		if [ `echo $?` != 0 ]
			then
				echo "Falha terminalseller-api - Health NOK- Host: $SRV" >> $FILE
		fi
	done
